...
k = int(input())
N = 2 ** k
col, row = list(map(int, input().split()))       # N*N 퍼즐, 구멍의 좌표
row = N - row
col = col - 1
data = [[0] * N for _ in range(N)]     # 초반의 퍼즐은 모두 -1, -1이 아닌 곳은 모두 구멍으로 간주
globals()['threshold'] = 2       # 2*2 퍼즐이 되면 빈 공간을 제외한 곳에 넣으면 됨
globals()['num'] = 1
data[row][col] = -1      # 구멍에 0을 넣음
...
def find_hole(data, n):
    ...
    m = n // 2
    row, col = [[row, col] for row in range(n) for col in range(n) if data[row][col] != 0][0]      # 구멍의 위치를 찾음
...
def func(data, n):
    ...
    if n <= globals()['threshold']:     # 2*2 보다 작아지면 빈 공간에 타일 넣으면 됨
        ...
        list(map(lambda x: func(x, m), [d1, d3, d2, d4]))      # 재귀
